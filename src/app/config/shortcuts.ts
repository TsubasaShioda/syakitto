/**
 * @file このファイルは、アプリケーション内で使用されるすべてのキーボードショートカットの定義と、
 * それらを分類するための情報を一元的に管理します。
 *
 * 主な内容：
 * - `ShortcutKey`型：各ショートカットが持つべきプロパティ（キー、修飾キー、説明、カテゴリー、アクションなど）を定義します。
 * - `shortcuts`配列：アプリケーション全体で利用可能な具体的なショートカット（一般操作、タイマー、姿勢検出など）をリストアップします。
 * - `categoryLabels`オブジェクト：ショートカットのカテゴリー内部名を、ユーザーインターフェースで表示するための日本語のラベルにマッピングします。
 */
// ショートカットキーの型定義
