/**
 * @file このファイルは、アプリケーションの「猫背検知」機能がどのように動作するか、
 * そして関連する設定項目が何を意味するのかを詳細に説明するためのReactコンポーネントです。
 *
 * 主な内容：
 * - 猫背検知の技術的な仕組み（AIによる目、耳、肩の位置分析と顔の大きさ補正）。
 * - ユーザーが調整可能な「猫背と判断するスコア」設定の解説。
 * - 「この秒数続いたら通知」設定の解説。
 * - ユーザーがアプリケーションをより効果的に利用するためのガイド。
 *
 * @component SlouchInfo
 * @returns {JSX.Element} 猫背検知機能に関する詳細情報と設定の解説を含むUI。
 */
"use client";
