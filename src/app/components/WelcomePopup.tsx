/**
 * @file このファイルは、アプリケーションの初回起動時に表示される、
 * 複数ページから成るウェルカムモーダルを定義するReactコンポーネントです。
 * ユーザーは「次へ」「戻る」ボタンやページネーションのドットを使って、
 * アプリケーションの主な機能や使い方を学ぶことができます。
 * 最後のページには「はじめる」ボタンが表示され、クリックするとモーダルが閉じられます。
 *
 * @component WelcomePopup
 * @param {boolean} isOpen - モーダルが開いているかどうか。
 * @param {() => void} onClose - モーダルを閉じるためのコールバック関数。
 *
 * @returns {JSX.Element | null} モーダルが開いている場合は、複数ページのウェルカムモーダルを返します。開いていない場合はnullを返します。
 */
"use client";
